...
like.post = function(req,res){
    ...
    conn.query(
        {
            sql: 'select userId from `secret_post_like` where postId = ' + ":id" + ' and userId='+req.session.userId,
            params:{
  ...
  if(r3.length==0) {
      ...
      conn.query(
          {
              sql: 'insert into secret_post_like (userId,postId,date) value ("' + req.session.userId + '","' + ":id" + '","' + common.time() + '")',
              params:{
                  id:parseInt(req.body.id)
              }
          }, function (e1, r1) {
                      ...
                      conn.query(
                          {
                              ...
                              'values ' +
                              '("' + req.session.userId + '","0","' + common.time() + '","1","1","' + req.session.nickname + '","","' + r2[0].content.substr(0, 128) + '","' + ":id" + '","' + r2[0].userId + '","0")',
                              params:{
                                  id:parseInt(req.body.id)
                              }
                          }, function (e2, r2) {
...
like.comment = function(req,res){
    conn.query(
        {
            sql:'insert into secret_comment_like (userId,commentId,date) value ("'+req.session.userId+'","'+":id"+'","'+common.time() +'")',
            params:{
                id:parseInt(req.body.id)
            }
        },function(e1,r1){
                    ...
                    conn.query(
                        {
                            sql:'insert into `secret_notice` (`userId`,`parentCommentId`,`date`,`status`,`pattern`,`nickname`,`content`,`originContent`,`postId`,`authorId`,`commentId`)' +
                                'values '+
                                '("'+req.session.userId+'","'+r2[0].parentId+'","' + common.time() + '","1","3","'+req.session.nickname+'","","'+r2[0].content.substr(0,128)+'","'+r2[0].postId+'","'+r2[0].userId+'","'+":id"+'")',
                            params:{
                                id:parseInt(req.body.id)
                            }
                        }, function (e2, r2) {
...
like.postsDel = function(req,res){
   ...
   //console.log('DELETE FROM `secret_post_like` WHERE postId = '+req.body.id);
    conn.query(
        {
            sql:'DELETE FROM `secret_post_like` where userId = '+req.session.userId+' and postId = ' + ":id",
            params:{
                id:parseInt(req.body.id)
            }
        },function(e1,r1){
...
like.commentDel = function(req,res){
    conn.query(
        {
            sql:'DELETE FROM `secret_comment_like` where userId = '+req.session.userId+' and commentId = ' + ":id",
            params:{
                id:parseInt(req.body.id)
            }
        },function(e1,r1){
